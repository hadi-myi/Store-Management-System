...
def shipment(store : int, delivery_date : str, reorders : list [int], shipment_items : dict, shipment_no : int, cnx : mysql.connector.connection):
    ...
    try: 
        with cnx.cursor() as cursor:
           ...
           # first_query = "SELECT REORDER_REQUEST.UPC, REORDER_REQUEST.quantity_requested, REORDER_REQUEST.reorder_id FROM REORDER_REQUEST WHERE REORDER_REQUEST.store_id = {} AND REORDER_REQUEST.reorder_id IN ({});".format(store, reorder_string)
            ...
            cursor.execute("SELECT REORDER_REQUEST.UPC, REORDER_REQUEST.quantity_requested, REORDER_REQUEST.reorder_id FROM REORDER_REQUEST WHERE REORDER_REQUEST.store_id = %s", ([store]))
